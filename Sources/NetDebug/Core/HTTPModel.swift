...
@objc public class HTTPModel: NSObject {
  ...
  func saveRequestBody(_ request: URLRequest) {
    saveRequestBodyData(request.getBody())
  }

  func logRequest(_ request: URLRequest) {
    formattedRequestLogEntry().appendToFile(filePath: Path.SessionLog)
  }
  ...
  func saveResponse(_ response: URLResponse, data: Data) {
    ...
    formattedResponseLogEntry().appendToFile(filePath: Path.SessionLog)
  }
  ...
  @objc public func getRequestBodyFilename() -> String {
    return String("_request_body_") + "\(requestTime!)_\(getRandomHash() as String)"
  }
  ...
  @objc public func getResponseBodyFilename() -> String {
    return String("_response_body_") + "\(requestTime!)_\(getRandomHash() as String)"
  }
  ...
  @objc public func saveData(_ dataString: NSString, toFile: String) {
    do {
      try dataString.write(
        toFile: toFile, atomically: false, encoding: String.Encoding.utf8.rawValue)
    } catch {
      print("catch !!!")
    }
  }
  ...
  public func prettyPrint(_ rawData: Data, type: HTTPModelShortType) -> NSString? {
    ...
    case .JSON:
      do {
        ...
        let prettyPrintedString = try JSONSerialization.data(
          withJSONObject: rawJsonData, options: [.prettyPrinted])
