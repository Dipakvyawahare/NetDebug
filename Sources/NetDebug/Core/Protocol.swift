...
@objc
open class Protocol: URLProtocol {
  static let InternalKey = "com.netfox.Internal"

  private lazy var session: URLSession = { [unowned self] in
    return URLSession(configuration: .default, delegate: self, delegateQueue: nil)
  }()

  private let model = HTTPModel()
  ...
  private class func canServeRequest(_ request: URLRequest) -> Bool {
    guard NetDebug.shared.isEnabled() else {
      return false
    }

    guard URLProtocol.property(forKey: Protocol.InternalKey, in: request) == nil,
      let url = request.url,
    ...
    guard !NetDebug.shared.getIgnoredURLs().contains(where: { absoluteString.hasPrefix($0) })
    else {
      return false
    }

    let regexMatches = NetDebug.shared.getIgnoredURLsRegexes()
  ...
  override open func startLoading() {
    ...
    URLProtocol.setProperty(true, forKey: Protocol.InternalKey, in: mutableRequest)
...
extension Protocol: URLSessionDataDelegate {
  ...
  public func urlSession(
    ...
    client?.urlProtocol(
      self, didReceive: response, cacheStoragePolicy: NetDebug.shared.cacheStoragePolicy)
  ...
  public func urlSession(
    ...
    HTTPModelManager.shared.add(model)
  ...
  public func urlSession(
    ...
    if URLProtocol.property(forKey: Protocol.InternalKey, in: request) != nil {
      let mutableRequest = (request as NSURLRequest).mutableCopy() as! NSMutableURLRequest
      URLProtocol.removeProperty(forKey: Protocol.InternalKey, in: mutableRequest)
