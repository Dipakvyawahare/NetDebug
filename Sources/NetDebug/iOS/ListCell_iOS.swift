import Foundation
  ...
  class ListCell: UITableViewCell {
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
      ...
      rightSeparator.backgroundColor = UIColor.LightGrayColor()
    ...
    override func layoutSubviews() {
      ...
      timeIntervalLabel.frame = CGRect(
        x: 0, y: requestTimeLabel.frame.maxY + 5, width: statusView.frame.width, height: 14)

      URLLabel.frame = CGRect(
        x: statusView.frame.maxX + padding, y: 0,
        width: frame.width - URLLabel.frame.minX - 25 - padding, height: 40)
      URLLabel.autoresizingMask = .flexibleWidth

      methodLabel.frame = CGRect(
        x: statusView.frame.maxX + padding, y: URLLabel.frame.maxY - 2, width: 40,
        height: frame.height - URLLabel.frame.maxY - 2)

      typeLabel.frame = CGRect(
        x: methodLabel.frame.maxX + padding, y: URLLabel.frame.maxY - 2, width: 180,
        height: frame.height - URLLabel.frame.maxY - 2)

      circleView.frame = CGRect(x: URLLabel.frame.maxX + 5, y: 17, width: 8, height: 8)

      leftSeparator.frame = CGRect(
        x: 0, y: frame.height - 1, width: statusView.frame.width, height: 1)
      rightSeparator.frame = CGRect(
        x: leftSeparator.frame.maxX, y: frame.height - 1,
        width: frame.width - leftSeparator.frame.maxX, height: 1)
    }
    ...
    func configForObject(_ obj: HTTPModel) {
    ...
    func setStatus(_ status: Int) {
      if status == 999 {
        statusView.backgroundColor = UIColor.Gray44Color()  //gray
        timeIntervalLabel.textColor = UIColor.white

      } else if status < 400 {
        statusView.backgroundColor = UIColor.GreenColor()  //green
        timeIntervalLabel.textColor = UIColor.DarkGreenColor()

      } else {
        statusView.backgroundColor = UIColor.RedColor()  //red
        timeIntervalLabel.textColor = UIColor.DarkRedColor()
      }
    }
    ...
    func isNewBasedOnDate(_ responseDate: Date) {
      if responseDate.isGreaterThanDate(NetDebug.shared.getLastVisitDate()) {
        isNew()
      } else {
        isOld()
      }
